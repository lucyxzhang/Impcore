    ...
    if (s->input == NULL) {
        ...
        case ATOM:
            synerror(s->context.source,
                     "%p: usage: (define f (x ...) [(locals x ...)] exp-body)",
                     s->context.par);
        case LIST:
            halfshift(s);
...
void *name_error(Par bad, struct ParsingContext *c) {
    ...
    case ADEF(VAL):
        synerror(c->source, "in %p, expected (val x exp), but %p is not a name",
                 c->par, bad);
    case ADEF(DEFINE):
        synerror(c->source, "in %p, expected (define f (x ...) [(locals x ...)] exp-body), "
                            "but %p is not a name",
                 c->par, bad);
    case ANXDEF(USE):
        synerror(c->source, "in %p, expected (use filename), "
                 ...
                 c->par, bad);
    case SET:
        synerror(c->source, "in %p, expected (set x exp), but %p is not a name",
                                                                                
                 c->par, bad);
    case APPLY:
        synerror(c->source, "in %p, expected (f ...), "
                            "but %p is not a name", 
                 c->par, bad);
    default:
